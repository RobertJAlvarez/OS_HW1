...
{
  size_t remainder, prev_remainder = ((size_t) 15); //We would always read the first character even if it don't match
  char guess_password[] = "a               "; //Password can't be greater on length than 16
  char options[] = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!@#$%&*_-+<>/"; //All the possible password character options
  short next_option = 1;
  int stat;

  while ((stat = try_password(&remainder, guess_password)) != 0) {
    if (stat == -1) { //If the function return -1 is because there was an error from the pipes and fds
      fprintf(stderr, "An error occurred. Cannot crack the password. Error: \"%s\"\n", strerror(errno));
      return 1;
    }
    //The 16 - remainder - 1 characters match so we must start changing the 16 - remaining character
    if (remainder != prev_remainder) {
      prev_remainder = remainder;
      next_option = 0;
    }
    guess_password[16-prev_remainder-1] = options[next_option++];
    //If we had try all 75 possible characters, the input password is not valid so we can't guess it
    if (next_option > (strlen(options)-1)) {
      printf("This code couldn't crack the password because it use a character outside of the 75 possible\n");
      return 1;
    }
  }

  //Trim the password
  for (char *p = guess_password; *p != '\0'; p++)
    if (*p == ' ')
      *p = '\0';
